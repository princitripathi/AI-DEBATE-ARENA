...
print("nvidia/nemotron-3-nano-30b-a3b:free")
print("=" * 56)
...
def run_debate(topic: str, rounds: int = 2, export: bool = False) -> None:
    pro = DebateAgent(
        name="Dr. Alex Chen",
        position="FOR",
    ...
    for round_num in range(1, rounds + 1):
        ...
        print(pro_arg)
        transcript_lines.append(f"**🟢 {pro.name} (FOR):** {pro_arg}\n")

        con_arg = con.make_argument(topic, round_num, pro_arg)
        print(f"\n🔴 {con.name} (AGAINST):")
        print(con_arg)
        transcript_lines.append(f"**🔴 {con.name} (AGAINST):** {con_arg}\n")
    ...
    if export:
        ...
        filename = f"transcripts/debate_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    ...
    parser.add_argument("--export", action="store_true", help="Save the full debate transcript + verdict as a Markdown file")
    args = parser.parse_args()

    rounds = max(1, min(4, args.rounds))
    run_debate(args.topic, rounds, export=args.export)
